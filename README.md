...
- **Database.** MariaDB 11.8, pinned in `traefik/templates/wp-template.yaml`. It is the engine the WP-CLI image can talk to: against MySQL 8.0 its MariaDB client rejects the server's self-signed certificate and cannot load `caching_sha2_password`, which breaks every `wp db` command. WordPress connects as the `wordpress` user, the same as in production. The `root` account still exists for administrative work: `docker compose exec mysql mariadb -uroot -prootpassword wordpress`.
